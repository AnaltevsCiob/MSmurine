...
---
```{r}
otu_table_HIT <- read.csv("~/Desktop/NTG17-060/HIT_rat/otu_table_HIT.csv")
...
```{r}
phy<-read.newick("/Volumes/backup/Ceca/Jan-2018-all/Desktop/HITdb-master/HITdb_v1.00/HITdb.tre")
...
###### now sample data
qiimeMap <- read.delim("~/Desktop/NTG17-060/qiimeMap.txt")
head(qiimeMap)
...
sample_data(phyloRatHIT)$ChildTime <- c("razdvajanje","12.dpi","majke","31-35.dpi","majke", "12.dpi","0.dpi","razdvajanje","0.dpi","31-35.dpi")
sample_data(phyloRatHIT)$ChildTime <- factor(sample_data(phyloRatHIT)$ChildTime, levels=c("majke", "razdvajanje", "0.dpi", "12.dpi", "31-35.dpi"))
```
...
ggplot(p$data, aes(x=time_point, y=value, color=AntibioticTreatment, group=AntibioticTreatment))+geom_point(size=3)+geom_line(linetype=2, size=1.5)+theme_bw()+labs(x="", y="Shannon index")+theme(axis.text.x=element_text(size=18), axis.title.y=element_text(size=18))
...
##
x <- aldex.clr(genusRatHIT[,2:11], mc.samples=128, verbose=TRUE)
